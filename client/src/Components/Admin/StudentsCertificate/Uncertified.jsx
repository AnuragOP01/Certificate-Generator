...
const Uncertified = ({setStudent}) => {
  ...
  const fetch = async () => {
    const res = await axiosInstance.get('/admin/unCertified')
    .then(res => {
  ...
  const verify = async () =>{
      const res = await axiosInstance.post('/certificate/certificate' , { ids : selectedIds})
      .then(async res => {
        console.log(res.data)
        await fetch();
      })
  ...
  const verifyOne = async (id) =>{    
    const res = await axiosInstance.post(`/certificate/certificate/${id}`)
    .then(async res => {
      console.log(res.data)
      await fetch();
    })
  ...
  return (
    <div className="w-full min-h-screen font-serif bg-gradient-to-t rounded-s-2xl from-blue-600 to-blue-700 text-white font-light text-2xl">
      ...
      <div className="flex flex-col font-medium text-2xl mt-10">
        <h1 className="text-center">All UnCertified Users</h1>
        <table>
          <thead>
