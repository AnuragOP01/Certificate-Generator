...
const Verified = ({setStudent}) => {
  ...
  const unverify = async () =>{
    const res = await axiosInstance.post('/admin/unverifyAll' , { ids : selectedIds})
    .then(res => {
      console.log(res.data)
      fetch();
    })
...
const verifyOne = async (id) =>{
  ...
  const res = await axiosInstance.post(`/admin/unverify/${id}`)
  .then(res => {
    console.log(res.data)
    fetch();
  })
  ...
  return (
    ...
    <div className="flex flex-col font-medium text-2xl mt-10">
      <h1 className="text-center">All Verified Users</h1>
      <table>
        <thead>
